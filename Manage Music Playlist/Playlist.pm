...
sub createPlaylist{
    ...
    if (exists($row->{$playlist_name})){
        ...
        print"Playlist Name is Inserted\n";
...
sub deletePlaylist{
    ...
    if (exists($row->{$delete_input})){
        ...
        print"Playlist Name is Deleted\n";
    }
    else{
        print"The Playlist Name is Invalid\n ";
    }
}
...
sub addSongsToDatabase{
    ...
    print"Song Inserted Sucessfully\n";
}
...
sub addSongPlaylist{
    ...
    if (exists($playlistNameRow->{$playlist_name})){        #Validating the inputs
        ...
        print"Invalid Playlist Name and song Entered \n ";
    }
}
...
sub fetchPlaylist{
    ...
    for my $keys(keys %row){
        for my $subkeys(keys %{$row{$keys}}){
            print "PlayList Names are:     ",$row{$keys}{$subkeys},"\n";
        }
    }
    # print Dumper($row);
...
sub shufflePlaylist{
    ...
    if (exists($validator->{$input_playlist_name})){
        ...
        print "Shuffled Play List\n", Dumper(\%shuffled_data);
        $object->{select_query}->finish();
    }
    else{
        print"Playlist name is invalid \n";
    }
}
