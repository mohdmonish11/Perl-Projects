...
while(1){
    ...
    print"Press 6 Fee Check for Enrolled Courses\n";
...
sub passDataToaddCourses{
    ...
    $object-> addCourses($course_name, $course_fee, $description);
}
        ...
        if (exists($student_data->{$student_id})){
            if (exists($course_data->{$course_id})){
                ...
                if ($fees_left_amount < 0){
                    ...
                    $object->coursesEnrollByStudent($course_id, $student_id, $fees_amount_paid, $fees_left_amount);
                } 
            }
            else{
        ...
        if(($data[2] == $id) or ($data[1] == $id)){
            ...
            if ($course_id == 0){
                ...
                if ($pending_amount >$data[4]){
                    ...
                    $object->updatePendingFee($course_id, $pending_amount, $student_id, @data);
                }
            }
            last;
        }
